...
$(function(){
    ...
    ajaxPost("/seimp/shareExchange/getDatalistByID", {
		...
		ajaxPost("/seimp/shareExchange/gpfygk/getLastUpdateTime", {
	    	ID:id
	    }).done(function (result) {
	    	if(result.status == 0){
	...
	function De2(){
		//更新表格
			...
			$('#hangyeTable').bootstrapTable({
			    method : 'POST',
			    url : "/seimp/shareExchange/gpfygk/getGPfygkData",
			    columns : columns,
			    //search:true,
...
function getGPfygkCons(){
	ajaxPost("/seimp/shareExchange/gpfygk/getGPfygkCons", {}).done(function (result) {
        $("#TJND").html("");
        $("#CPHYLMC").html("");
        $("#TJND").append('<option value="">全部</option>');
        $("#CPHYLMC").append('<option value="">全部</option>');
        for (var i = 0; i < result.data.tjnd.length; i++) {

            var currItem = result.data.tjnd[i];
            $("#TJND").append('<option value="' + currItem.TJND + '">' + currItem.TJND + '</option>');
        }
        for (var i = 0; i < result.data.cphylmc.length; i++) {
            var currItem = result.data.cphylmc[i];
            if (currItem && currItem.CPHYLMC != "(null)") {
                $("#CPHYLMC").append('<option value="' + currItem.CPHYLMC + '">' + currItem.CPHYLMC + '</option>');
            }
        }
    })
}
...
function De5(){
	ajaxPost("/seimp/shareExchange/gpfygk/getStatisDataByUpdateTime", {
...
function kepGetExcelFile(){
	...
	ajaxPost("/seimp/shareExchange/gpfygk/getExcelFile", parameter).done(function (result) {
		//删除等待提示框
