			...
			var columns =[{
		        ...
		        formatter: function (value, row, index) {
		            ...
		            formatter :function(value,row,inde){
		                ...
		                s += "&nbsp;&nbsp;&nbsp;&nbsp;<a  onclick=showDetailsModal(\""+row.id+"\",\""+row.ksmc+"\"); >详情</a>";
		                return s;
		            }
		        }];
			//销毁表格
...
function De1(){
	...
	ajaxPost("/seimp/shareExchange/mlr/getStatisDataOfHyname", {}).done(function (res) {
...
//    		        boundaryGap: [0, 0.01],
	...
	//数据概览-省份统计
    /*
    ajaxPost("/seimp/shareExchange/mlr/getStatisDataOfProvince", {}).done(function (res) {
    	//console.log(res.data);
    	var names = [];
    	var num=[];
...
function kepGetExcelFile(){
	...
	ajaxPost("/seimp/shareExchange/mlr/getExcelFile", parameter).done(function (result) {
		//删除等待提示框
...
function showDetailsModal(id, ksmc){
	var enterPriseInfo = {
			id:id
		};
	parent.showDetailsModal(JSON.stringify(enterPriseInfo), ksmc, "views/data/detailIframe/mlrDetails.html")
}
