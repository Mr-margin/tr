...
$(function(){
    ...
    ajaxPost("/seimp/shareExchange/getDatalistByID", {
		...
		ajaxPost("/seimp/shareExchange/organCode/getLastUpdateTime", {
	    	ID:id
	    }).done(function (result) {
	    	if(result.status == 0){
	...
	function De2(){
		//更新表格
			var columns =[{
		        ...
		        class:'w2-125',
		        formatter: function (value, row, index) {
	            	return "<div ><span style='width:125px;' title='"+value+"'>"+value+"</span></div>";
	        	}
		    }, {
		        field: 'JGDM0',
		        title: '组织机构代码',
		        ...
		        class:'w2-70',
		        formatter: function (value, row, index) {
	            	return "<div ><span style='width:70px;' title='"+value+"'>"+value+"</span></div>";
	        	}
		    }, {
		        field: 'CITY',
		        title: '市区',
		        ...
		        class:'w2-90',
		        formatter: function (value, row, index) {
	            	return "<div ><span style='width:90px;' title='"+value+"'>"+value+"</span></div>";
	        	}
	        }, /*{
		        field: 'BGRQ0',
		        title: '最后变更日期',
		        ...
		        align: 'center',
		            ...
		            formatter :function(value,row,inde){
		                ...
		                var s = "<a class='"+className+"'  onclick=dingwei('" + row.LON +"','"+ row.LAT +"','"+ row.JGMC0 + "');>定位</a>";
		                s += "&nbsp;&nbsp;&nbsp;&nbsp;<a   onclick=showDetailsModal(\""+encodeURI(row.JGDM0)+"\",\""+row.JGMC0+"\"); >详情</a>";
		                return s;
		            }
		        }];
			//销毁表格
			...
			$('#hangyeTable').bootstrapTable({
			    method : 'POST',
			    url : "/seimp/shareExchange/organCode/getOrganCodeData",
			    columns : columns,
			    //search:true,
...
//数据概览-按行业统计
function De1(){
	ajaxPost("/seimp/shareExchange/organCode/getStatisDataOfHyname", {}).done(function (res) {
...
//    		        boundaryGap: [0, 0.01],
    ...
    //判断用户级别
	var staUrl = "/seimp/shareExchange/organCode/getStatisDataOfProvince";
	var staYAxisName = "省份名称";
	var userLevel = sessionStorage["userLevel"];
	if(userLevel && userLevel=="2"){
		staUrl = "/seimp/shareExchange/organCode/getStatisDataOfCity";
		staYAxisName = "市区名称";
		$("#staDivName").html("<i></i>按市区统计组织机构代码");
	}
...
function De5(){
	ajaxPost("/seimp/shareExchange/organCode/getStatisDataByUpdateTime", {
...
//详情信息
function showDetailsModal(JGDM0, JGMC0){
	JGDM0 = decodeURI(JGDM0);
	var enterPriseInfo = {
			JGDM0:JGDM0
		};
	parent.showDetailsModal(JSON.stringify(enterPriseInfo), JGMC0, "views/data/detailIframe/organCodeDetails.html")
}