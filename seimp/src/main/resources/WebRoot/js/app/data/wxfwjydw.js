...
$(function(){
    ...
    ajaxPost("/seimp/shareExchange/getDatalistByID", {
		...
		ajaxPost("/seimp/shareExchange/wxfwjydw/getLastUpdateTime", {
	    	ID:id
	    }).done(function (result) {
	    	if(result.status == 0){
		...
		//更新表格
			var columns =[{
	            ...
	            formatter :function(value,row,inde){
	            	...
	            	if(row.SSJD && row.SSWD){
	            		className = "successDingwei";
	            	}
	                var s = "<a class='"+className+"'  onclick=dingwei('" + getLonLat(row.SSJD) +"','"+ getLonLat(row.SSWD) +"','"+ row.DWMC + "');>定位</a>";
	                return s;
	            }
	        }];
			//销毁表格
			...
			$('#hangyeTable').bootstrapTable({
			    method : 'POST',
			    url : "/seimp/shareExchange/wxfwjydw/getWxfwjydwData",
			    columns : columns,
			    //search:true,
...
function getLonLat(value){
	...
	if(value){
		...
		if(arr.length > 1){
			result = parseFloat(result) + parseFloat((parseFloat(arr[1])/60).toFixed(4));
		}
		if(arr.length > 2){
			result = parseFloat(result) + parseFloat((parseFloat(arr[2])/3600).toFixed(4));
		}
	}
	return result;
}
...
function De5(){
	ajaxPost("/seimp/shareExchange/wxfwjydw/getStatisDataByUpdateTime", {
...
function kepGetExcelFile(){
	...
	ajaxPost("/seimp/shareExchange/wxfwjydw/getExcelFile", parameter).done(function (result) {
		//删除等待提示框
