...
$(function(){
    ...
    ajaxPost("/seimp/shareExchange/getDatalistByID", {
		...
		ajaxPost("/seimp/shareExchange/wy/getLastUpdateTime", {
	    	ID:id
	    }).done(function (result) {
	    	if(result.status == 0){
...
//	            class :'w-70'
			    ...
			    valign: 'middle' 
		            ...
		            formatter :function(value,row,inde){
		                ...
		                var s = "<a class='"+className+"'  onclick=dingwei('" + row.LONGITUDE +"','"+ row.LATITUDE +"','"+ row.TAILINGSNAME + "');>定位</a>";
		                s += "&nbsp;&nbsp;&nbsp;&nbsp;<a  onclick=showDetailsModal(\""+row.OID+"\",\""+row.NAME+"\"); >详情</a>";
		                return s;
		            }
		        }];
			//销毁表格
			...
			$('#hangyeTable').bootstrapTable({
			    method : 'POST',
			    url : "/seimp/shareExchange/wy/getWYData",
			    columns : columns,
			    //search:true,
...
//			    onlyInfoPagination:false,
			    queryParams: function queryParams(params) {   //设置查询参数  
			    	...
			    	var data =  JSON.stringify({
			    		...
			    		name : $("#name").val(),//企业名称
			    		province : $("#province").val()==null ? "" : $("#province").val(),//所属行政区划
			    		status : $("#status").val()==null ? "" : $("#status").val(),//核查状态
			    		production : $("#production").val()==null ? "" : $("#production").val(),//是否在产
			    		progress : $("#progress").val()==null ? "" : $("#progress").val(),//核查进度
				    });	    	
					datas.data = data;
				    return datas;  
				    console.log(datas)
...
//数据概览-按行业统计
function De1(){
	ajaxPost("/seimp/shareExchange/wy/getStatisDataOfHyname", {}).done(function (res) {
...
//    		        boundaryGap: [0, 0.01],
    ...
    //判断用户级别
	var staUrl = "/seimp/shareExchange/wy/getStatisDataOfProvince";
	var staYAxisName = "省份名称";
	var userLevel = sessionStorage["userLevel"];
	if(userLevel && userLevel=="2"){
		staUrl = "/seimp/shareExchange/wy/getStatisDataOfCity";
		staYAxisName = "市区名称";
		$("#staDivName").html("<i></i>按市区统计重点行业企业遥感核查");
	}
...
function De5(){
	ajaxPost("/seimp/shareExchange/wy/getStatisDataByUpdateTime", {
...
function kepGetExcelFile(){
	...
	ajaxPost("/seimp/shareExchange/wy/getExcelFile", parameter).done(function (result) {
		//删除等待提示框
...
//详情信息
function showDetailsModal(OID, NAME){
	var enterPriseInfo = {
			OID:OID
		};
	parent.showDetailsModal(JSON.stringify(enterPriseInfo), NAME, "views/data/detailIframe/wyDetails.html")
}
