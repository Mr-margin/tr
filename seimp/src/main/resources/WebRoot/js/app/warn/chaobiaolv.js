...
var wuRanObject = ["cr","pb","cd","hg","as","cu","zn","ni"];//污染物类型，默认所有类型

var check_val = [];
/********页面加载**********/
$(function () {
    //污染类型按钮
    $("#xiangmuDiv label.leiXing input").click(function (tem) {
        lastWuranType = wuranType;
        if (this.value == "totalCount") {
            if (this.checked) {
                $("#xiangmuDiv label.leiXing input").prop("checked", true);
            } else {
                $("#xiangmuDiv label.leiXing input").prop("checked", false);
            }
        }
        ...
        for (k in obj) {
            if (obj[k].checked)
                wuranType.push(obj[k].value);
        }
        if (wuranType.contain("SLIGHT") && wuranType.contain("LIGHT") && wuranType.contain("MIDDLE") && wuranType.contain("SERVERE")) {
            $("#totalCount").prop("checked", true);
        } else {
        ...
        if (wuranType.contain("totalCount")) {
     ...
     * 污染物多选按钮点击事件
     */
    $("#xiangmuDiv label.wuRanObject input").click(function(evt){
    	//全部按钮
    	if (this.value == "totalObject") {
            if (this.checked) {
                $("#xiangmuDiv label.wuRanObject input").prop("checked", true);
            } else {
                $("#xiangmuDiv label.wuRanObject input").prop("checked", false);
            }
        }else{
        	if (!this.checked) {
        		//如果其他的按钮去掉一个，全部按钮选中状态取消
        		$("#wuRanObjectTotal").prop("checked", false);
        	}
        	if($("#xiangmuDiv label.wuRanObject input:checked").length==8){
        		//如果其他按钮都选中，全部按钮自动选中
        		$("#wuRanObjectTotal").prop("checked", true);
        	}
        }
    	wuRanObject = [];
    	//选中的值
    	$.each($("#xiangmuDiv label.wuRanObject input[name='choiceObject']:checked"),function(index,obj){
    		wuRanObject.push($(obj).val());
    	});
...
function switchIndustry(value) {
    ...
    if (value == "1") {//无机物
    	//显示污染物泪类型
    	$("#xiangmuDiv span:eq(1)").show();
    	$("#xiangmuDiv  label.wuRanObject").show();
    	
        $("#mapItem li:eq(0)").addClass("metal");
    } else {//有机物
    	$("#xiangmuDiv span:eq(1)").hide();
    	$("#xiangmuDiv label.wuRanObject").hide();
...
//			map.graphics.clear();
        ...
        //设置分段渲染
        //ajax获取各省产粮油大县的数量
        var json = {type: industryPar,wuRanObject:wuRanObject.join(",")};
            ...
            if (result.status == 0) {//请求成功
                if (result.data.length > 0) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        var tem = 0;
                        for (var k = 0; k < wuranType.length; k++) {
                            tem += data[i][wuranType[k]];
                        }
                        if (tem < minCount) {
                            minCount = tem;
                        }
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        var tem = 0;
                        for (var k = 0; k < wuranType.length; k++) {
                            tem += currItem[wuranType[k]];
                        }
                        if (minCount <= tem && tem < minCount + count * 1) {
                            renderer.addValue(currItem.CODE, symbol1);
                        } else if (minCount + count * 1 <= tem && tem < minCount + count * 2) {
                            renderer.addValue(currItem.CODE, symbol2);
                        } else if (minCount + count * 2 <= tem && tem < minCount + count * 3) {
                            renderer.addValue(currItem.CODE, symbol3);
                        } else if (minCount + count * 3 <= tem && tem < minCount + count * 4) {
                            renderer.addValue(currItem.CODE, symbol4);
                        } else if (minCount + count * 4 <= tem && tem <= minCount + count * 5) {
                            renderer.addValue(currItem.CODE, symbol5);
                        } else if (minCount + count * 5 <= tem && tem <= minCount + count * 6) {
                ...
                for (var j = 0; j < data.length; j++) {
                    ...
                    if (currFeaGraphic.attributes.PROV_CODE == currItem.CODE) {
                        ...
                        if (tem.toString().length == 1) {
                            symbolSize = 24;
                        } else if (tem.toString().length == 2) {
                            symbolSize = 30;
                        } else if (tem.toString().length == 3) {
                            symbolSize = 40;
                        } else if (tem.toString().length == 4) {
                            symbolSize = 50;
                        } else if (tem.toString().length == 5) {
                            symbolSize = 60;
                        } else {
                            symbolSize = tem.toString().length * 12;
                        }

                        var symbol1 = new PictureMarkerSymbol("../../img/information/number.png", symbolSize, symbolSize).setOffset(0, symbolSize / 2);
                        var symbol2 = new TextSymbol(tem).setOffset(0, symbolSize / 2 - 5).setColor(new Color([255, 255, 255, 1]));//.setHaloSize(3).setHaloColor(new Color([255,255,255]));
                        symbol2.setText(tem);
...
//			$("#city").addClass("none");
        ...
        //设置分段渲染
        var json = {type: industryPar, regionCode: provinceCode,wuRanObject:wuRanObject.join(",")};
...
//			map.graphics.clear();
        ...
        //设置分段渲染
        var json = {type: industryPar, regionCode: cityCode,wuRanObject:wuRanObject.join(",")};
