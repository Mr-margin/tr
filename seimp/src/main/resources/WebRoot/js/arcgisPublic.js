...
//         addTiandituVectorLayer();
       ...
       //app.map.on("mouse-drag-end", resizess)//拖动
        ...
        // app.map.reorderLayer(app.baseLayer,0)
        // app.map.reorderLayer(app.annoLayer,1)
        ...
        var herfParam = herf.split("=")[1];
        if (herfParam) {
        	if (herfParam == "zhongdianqiye") {
        		$(".soilDl label")[6].click();
        	} else if (herfParam =="dikuai"){
        		$(".wurandikuai label")[1].click();
        	}
        }
        allPublic();
    })
...
function createGoogleLayer(){
    ...
    // app.map.getLayersVisibleAtScale()[2].setVisibility(google_show);
}
...
function  wangge (){//网格线
    ...
    app.grideGraphicLayer=new dong.GraphicsLayer({id:"wangmian"});   //定义一个存放格网的全局变量
...
function sc_mian(){
    removeTc("wangxian")
    app.wanggeLayer = new dong.GraphicsLayer({id:"wangxian"});
    ...
    for ( var i = 0 ; i <  dong.oEndRow* dong.oEndColumn; i ++ ) {
        ...
        var sfs = new dong.SimpleFillSymbol(dong.SimpleFillSymbol.STYLE_SOLID,
            new dong.SimpleLineSymbol(dong.SimpleLineSymbol.STYLE_SOLID,
                new dong.Color([255,0,0]),1),new dong.Color([0,0,0,0])
        );
...
function resizess(event){
    if(!document.getElementById("wg").checked) return;
    var left_x = event.xmin-500000;
    var left_y = event.ymin-500000;
    if ( isNaN(left_x) ){
       left_x = event.mapPoint.x-1000000;
       left_y = event.mapPoint.y-1000000;
       if(dong.resizess == "1")  resizess1();
       if(dong.resizess == "2")  resizess2(left_x,left_y);
       if(dong.resizess == "3")  resizess3(left_x,left_y);
     }
    if(app.map.getScale().toFixed()>=9244659){//省
        ...
        if(app.map.getScale().toFixed()>4622323 ) {//市
            dong.resizess =2;
            resizess2(left_x,left_y);
        } else{//县
            dong.resizess =3;
            resizess3(left_x,left_y);
        }
    }
}
//缩放一级
...
//缩放二级
function resizess2(left_x,left_y){
...
//缩放三级
function resizess3(left_x,left_y){
    for ( var i = 0 ; i < dong.mian_point.length;i ++ ) {
        var str = dong.data[i];
        var x1,x2,y1,y2;
        if ( str.point1.x > str.point2.x ){
