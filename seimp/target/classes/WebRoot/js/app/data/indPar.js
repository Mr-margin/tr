	...
	function De2(){
		//更新表格
			...
			$('#hangyeTable').bootstrapTable({
			    method : 'POST',
			    url : "/seimp/shareExchange/indPar/getIndParData",
			    columns : columns,
			    //search:true,
...
function getIndParCon(){
	ajaxPost("/seimp/shareExchange/indPar/getIndParCon", {}).done(function (result) {
        if (result.status == 0) {
 	       var html="";
...
function kepGetExcelFile(){
	...
	ajaxPost("/seimp/shareExchange/indPar/getExcelFile", parameter).done(function (result) {
		//删除等待提示框
