...
$(function(){
    ...
    ajaxPost("/seimp/shareExchange/getDatalistByID", {
		...
		ajaxPost("/seimp/shareExchange/jubao12369/getLastUpdateTime", {
	    	ID:id
	    }).done(function (result) {
	    	if(result.status == 0){
	    		if(result.data){
	...
	function De2(){
		//更新表格
			var columns =[{
		        ...
		        class : 'w2-100',
		        formatter: function (value, row, index) {
	            	return "<div ><span style='width:150px;' title='"+value+"'>"+value+"</span></div>";
	        	}
		    }, {
		        field: 'PROVINCE',
		        title: '对接省份',
		        ...
		        valign: 'middle',
		            ...
		            formatter :function(value,row,inde){
		                ...
		                s += "&nbsp;&nbsp;&nbsp;&nbsp;<a  onclick=showDetailsModal(\""+row.REPORT_ID+"\",\""+row.REPORT_DEPT_NAME+"\"); >详情</a>";
		                return s;
		            }
		        }];
			//销毁表格
			...
			$('#hangyeTable').bootstrapTable({
			    method : 'POST',
			    url : "/seimp/shareExchange/jubao12369/getJubaoData",
			    columns : columns,
			    //search:true,
...
//			    onlyInfoPagination:false,
			    queryParams: function queryParams(params) {   //设置查询参数  
			    	...
			    	var data =  JSON.stringify({
			    		...
			    		reportDeptName : $("#reportDeptName").val(),
...
function De1(){
	...
	ajaxPost("/seimp/shareExchange/jubao12369/getStatisDataOfHyname", {}).done(function (res) {
...
//    		        formatter: "{a} <br/>{b} : {c} ({d}%)"
    		        ...
    		        formatter: function (params){                                                                                                                       
                        var result = params.seriesName+'<br>'
                        	...
                        	+params.data.name+': '+params.data.value+"(条)"+params.percent+'%';                                                                         
                        return result;                                                                                                                                  
                    }
    		    },
    		    legend: {
    ...
    //判断用户级别
	var staUrl = "/seimp/shareExchange/jubao12369/getStatisDataOfProvince";
	var staYAxisName = "省份名称";
	var userLevel = sessionStorage["userLevel"];
	if(userLevel && userLevel=="2"){
		staUrl = "/seimp/shareExchange/jubao12369/getStatisDataOfCity";
		staYAxisName = "市区名称";
		$("#staDivName").html("<i></i>按市区统计12369举报预警");
	}
...
function De5(){
	ajaxPost("/seimp/shareExchange/jubao12369/getStatisDataByUpdateTime", {
...
function kepGetExcelFile(){
	...
	ajaxPost("/seimp/shareExchange/jubao12369/getExcelFile", parameter).done(function (result) {
		//删除等待提示框
...
//详情信息
function showDetailsModal(REPORT_ID, REPORT_DEPT_NAME){
	var enterPriseInfo = {
			REPORT_ID:REPORT_ID
		};
	parent.showDetailsModal(JSON.stringify(enterPriseInfo), REPORT_DEPT_NAME, "views/data/detailIframe/jubao12369Details.html")
}
