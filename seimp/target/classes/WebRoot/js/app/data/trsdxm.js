...
$(function(){
    ...
    ajaxPost("/seimp/shareExchange/getDatalistByID", {
		...
		ajaxPost("/seimp/shareExchange/trsdxm/getLastUpdateTime", {
	    	ID:id
	    }).done(function (result) {
	    	if(result.status == 0){
		...
		//更新表格
			var columns =[{
	            ...
	            formatter :function(value,row,inde){
	                ...
	                var s = "<a class='"+className+"'  onclick=dingwei('" + row.LON +"','"+ row.LAT +"','"+ row.XMMC + "');>定位</a>";
	                s += "&nbsp;&nbsp;&nbsp;&nbsp;<a   onclick=showDetailsModal(\""+encodeURI(row.ID)+"\",\""+row.XMMC+"\"); >详情</a>";
	                return s;
	            }
			...
			$('#hangyeTable').bootstrapTable({
			    method : 'POST',
			    url : "/seimp/shareExchange/trsdxm/getTrsdxmData",
			    columns : columns,
			    //search:true,
...
function De1(){
	...
	ajaxPost("/seimp/shareExchange/trsdxm/getStatisDataOfHyname", {}).done(function (res) {
    	...
    	if(res.data.length<20){
         ...
         for (var i = 0; i < length; i++) {
	        	...
	        	if(item.name!=undefined && item.name!="" && item.name!=null && item.name!=" "){        		
	        		name.push(item.name);
		            vals.push({value:item.COUNT,name:item.name})
	        	}
	      }
...
//    		        formatter: "{a} <br/>{b} : {c} ({d}%)"
    		        ...
    		        formatter: function (params){                                                                                                                       
                        var result = params.seriesName+'<br>'
                        	...
                        	+params.data.name+': '+params.data.value+"(个)"+params.percent+'%';                                                                         
                        return result;                                                                                                                                  
                    }
    		    },
  ...
  //判断用户级别
	var staUrl = "/seimp/shareExchange/trsdxm/getStatisDataOfProvince";
	var staYAxisName = "省份名称";
	var userLevel = sessionStorage["userLevel"];
	if(userLevel && userLevel=="2"){
		staUrl = "/seimp/shareExchange/trsdxm/getStatisDataOfCity";
		staYAxisName = "市区名称";
...
function getHQyjbxxCons(){
	ajaxPost("/seimp/shareExchange/trsdxm/getTrsdxmCons", {}).done(function (result) {
        $("#XMLX").html("");
        $("#XMLX").append('<option value="">全部</option>');
        
        for (var i = 0; i < result.data.length; i++) {
            var currItem = result.data[i];
            $("#XMLX").append('<option value="' + currItem.XMLX + '">' + currItem.XMLX + '</option>');
        }
...
function De5(){
	ajaxPost("/seimp/shareExchange/trsdxm/getStatisDataByUpdateTime", {
...
//    		DIZHI : $("#DIZHI").val(),
	...
	ajaxPost("/seimp/shareExchange/trsdxm/getExcelFile", parameter).done(function (result) {
		//删除等待提示框
...
//详情信息
function showDetailsModal(ID, XMMC){
	ID = decodeURI(ID);
	var enterPriseInfo = {
			ID:ID
		};
	parent.showDetailsModal(JSON.stringify(enterPriseInfo), XMMC, "views/data/detailIframe/trsdxmDetails.html")
}