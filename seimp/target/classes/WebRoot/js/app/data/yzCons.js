...
$(function(){
    ...
    ajaxPost("/seimp/shareExchange/getDatalistByID", {
		...
		ajaxPost("/seimp/shareExchange/yzCons/getLastUpdateTime", {
	    	ID:id
	    }).done(function (result) {
	    	if(result.status == 0){
	...
	$("#eiamanagename").selectpicker("refresh");
	$("#eiamanagename").selectpicker("val",[""]);
	$("#eiamanagename").on("changed.bs.select",function (e, clickedIndex, isSelected, previousValue) {
		//如果是全选，去掉其他
		...
		//如果是其他，去掉全选
		if(clickedIndex!=0){
			var currVal = $("#eiamanagename").selectpicker("val");
			if(currVal[0]==""){
				currVal.splice(0, 1);
				$("#eiamanagename").selectpicker("val", currVal);
			}
		}
	});
...
//		            class:'w-100'
		            ...
		            formatter :function(value,row,inde){
		                ...
		                s += "&nbsp;&nbsp;&nbsp;&nbsp;<a  onclick=showDetailsModal(\""+row.ID+"\",\""+row.PROJECTNAME+"\"); >详情</a>";
		                return s;
		            }
		        }];
			//销毁表格
			...
			$('#hangyeTable').bootstrapTable({
			    method : 'POST',
			    url : "/seimp/shareExchange/yzCons/getYzConsData",
			    columns : columns,
			    //search:true,
...
//			    onlyInfoPagination:false,
			    queryParams: function queryParams(params) {   //设置查询参数
			    	var nationVal = $("#nationalecomomyname").selectpicker("val")==null ? "" :   "'" +$("#nationalecomomyname").selectpicker("val").join("','")+"'";
			    	var eiaVal = $("#eiamanagename").selectpicker("val")==null ? "" :    $("#eiamanagename").selectpicker("val").join(",");
			    	var datas = {};
			    	var data =  JSON.stringify({
			    		...
			    		provincename : $("#provincename option[value='"+ $("#provincename").val() +"']").attr("selectValue"),
			    		eiamanagename : eiaVal,
			    		nationalecomomyname : nationVal,
			    		datasource : $("#datasource").val()==null?'':$("#datasource").val(),
			    		acceptancedateStart : $("#acceptancedateStart").val(),
			    		acceptancedateEnd : $("#acceptancedateEnd").val()
				    });	    	
					datas.data = data;
				    return datas;  
				    console.log(datas)
...
function De1(){
	/*
	ajaxPost("/seimp/shareExchange/yzCons/getStatisDataOfHyname", {}).done(function (res) {
    ...
    */
	...
	if(userLevel && userLevel=="2"){
		$("#staDivName").parent().parent().hide();
		$("#staLi1").hide();
		//判断是否有查询权限
		if($("#selectLi").is(":visible")){
			$("#selectLi").click();
		}else{
			$(".sjTabs li:eq(2)").click();
		}
    ...
    ajaxPost(staUrl, {}).done(function (res) {
    	...
    	var maxValue = 0;
    	 for (var i = 0; i < res.data.length; i++) {
	        	var item = res.data[i];
	        	if(item.name!=undefined && item.name!="" && item.name!=null && item.name!=" " && item.name.indexOf("、")==-1){	        		
...
//全部行业
function industry(){
	 ajaxPost("/seimp/shareExchange/yzCons/getAllDatasource", {}).done(function (result) {
	    	console.log(result.data);
	    	var dataSource = result.data.dataSource;
	    	var html="";
	    	html += "<option value=''>全部</option>";
	    	for(var i=0;i<dataSource.length;i++){
	    		if(dataSource[i].DATASOURCE!=undefined && dataSource[i].DATASOURCE!="" && dataSource[i].DATASOURCE!=null && dataSource[i].DATASOURCE!=" "){		        		
	    			html += "<option value='"+dataSource[i].DATASOURCE+"'>"+dataSource[i].DATASOURCE+"</option>";
	    		}
	    	}
	    	$("#datasource").html(html)
	    	
	    	var nationalecomomyname = result.data.nationaleconomyname;
	    	var html2="";
	    	html2 += "<option value=''>全部</option>";
	    	for(var i=0;i<nationalecomomyname.length;i++){
	    		if(nationalecomomyname[i].NATIONALECONOMYNAME!=undefined && nationalecomomyname[i].NATIONALECONOMYNAME!="" && nationalecomomyname[i].NATIONALECONOMYNAME!=null && nationalecomomyname[i].NATIONALECONOMYNAME!=" "){		        		
	    			html2 += "<option value='"+nationalecomomyname[i].NATIONALECONOMYNAME+"'>"+nationalecomomyname[i].NATIONALECONOMYNAME+"</option>";
	    		}
	    	}
	    	$("#nationalecomomyname").html(html2);
	    	$("#nationalecomomyname").selectpicker("refresh");
	    	$("#nationalecomomyname").selectpicker("val",[""]);
	    	$("#nationalecomomyname").on("changed.bs.select",function (e, clickedIndex, isSelected, previousValue) {
	    		//如果是全选，去掉其他
	    		if(clickedIndex==0 && isSelected==true){
	    			$("#nationalecomomyname").selectpicker("val",[""]);
	    		}
	    		//如果是其他，去掉全选
	    		if(clickedIndex!=0){
	    			var currVal = $("#nationalecomomyname").selectpicker("val");
	    			if(currVal[0]==""){
	    				currVal.splice(0, 1);
	    				$("#nationalecomomyname").selectpicker("val", currVal);
	    			}
	    		}
	    	});
...
function De5(){
	ajaxPost("/seimp/shareExchange/yzCons/getStatisDataByUpdateTime", {
...
function kepGetExcelFile(){
	...
	var parameter = {
			projectname : $("#projectname").val(),
    		provincename : $("#provincename option[value='"+ $("#provincename").val() +"']").attr("selectValue"),
    		eiamanagename : $("#eiamanagename").selectpicker("val"),
    		datasource : $("#datasource").val()==null?'':$("#datasource").val(),
    		acceptancedateStart : $("#acceptancedateStart").val(),
    		acceptancedateEnd : $("#acceptancedateEnd").val(),
    		metadataID : id
	}
	ajaxPost("/seimp/shareExchange/yzCons/getExcelFile", parameter).done(function (result) {
		//删除等待提示框
...
//详情信息
function showDetailsModal(CONSTRUCTIONID, PROJECTNAME){
	var enterPriseInfo = {
			CONSTRUCTIONID:CONSTRUCTIONID
		};
	parent.showDetailsModal(JSON.stringify(enterPriseInfo), PROJECTNAME, "views/data/detailIframe/yzConsDetails.html")
}
