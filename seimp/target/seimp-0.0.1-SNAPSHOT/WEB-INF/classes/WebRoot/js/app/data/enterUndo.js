...
$(function(){
    ...
    ajaxPost("/seimp/shareExchange/getDatalistByID", {
		...
		ajaxPost("/seimp/shareExchange/enterUndo/getLastUpdateTime", {
	    	ID:id
	    }).done(function (result) {
	    	if(result.status == 0){
	...
	function De2(){
		//更新表格
			...
			$('#hangyeTable').bootstrapTable({
			    method : 'POST',
			    url : "/seimp/shareExchange/enterUndo/getEnterpriseUndoinfoData",
			    columns : columns,
			    //search:true,
...
//			    onlyInfoPagination:false,
			    queryParams: function queryParams(params) {   //设置查询参数  
			    	...
			    	var data =  JSON.stringify({
			    		...
			    		isType : $("#isType").val()==null?'':$("#isType").val(),
			    		unitName : $("#unitName").val()==null?'':$("#unitName").val(),
			    		zxType : $("#zxType").val()==null?'':$("#zxType").val(),
			    		createTimeStart : $("#createTimeStart").val()==null?'':$("#createTimeStart").val(),
			    		createTimeEnd : $("#createTimeEnd").val()==null?'':$("#createTimeEnd").val(),
				    });	    	
					datas.data = data;
				    return datas;  
				    console.log(datas)
...
//数据概览-按行业统计
function De1(){
	ajaxPost("/seimp/shareExchange/enterUndo/getStatisDataOfHyname", {}).done(function (res) {
...
//    		        boundaryGap: [0, 0.01],
    ...
    //判断用户级别
	var staUrl = "/seimp/shareExchange/enterUndo/getStatisDataOfProvince";
	var staYAxisName = "省份名称";
	var userLevel = sessionStorage["userLevel"];
	if(userLevel && userLevel=="2"){
		staUrl = "/seimp/shareExchange/enterUndo/getStatisDataOfCity";
		staYAxisName = "市区名称";
		$("#staDivName").html("<i></i>按市区统计排污许可证撤销、注销企业");
	}
...
//全部行业
function industry(){
	 ajaxPost("/seimp/shareExchange/enterUndo/getAllZXTYPEByZXTYPE", {}).done(function (result) {
	    	console.log(result.data);
	    	var html="";
	    	html += "<option value=''>全部</option>";
	    	for(var i=1;i<result.data.length;i++){
	    		if(result.data[i].ZXTYPE!=undefined && result.data[i].ZXTYPE!="" && result.data[i].ZXTYPE!=null && result.data[i].ZXTYPE!=" "){		        		
	    			html += "<option value='"+result.data[i].ZXTYPE+"'>"+result.data[i].ZXTYPE+"</option>";
	    		}
	    	}
	    	$("#zxType").html(html)
...
function De5(){
	ajaxPost("/seimp/shareExchange/enterUndo/getStatisDataByUpdateTime", {
...
function kepGetExcelFile(){
	...
	ajaxPost("/seimp/shareExchange/enterUndo/getExcelFile", parameter).done(function (result) {
		//删除等待提示框
