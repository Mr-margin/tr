...
$(function(){
    ...
    ajaxPost("/seimp/shareExchange/getDatalistByID", {
		...
		ajaxPost("/seimp/shareExchange/gqyjbxx/getLastUpdateTime", {
	    	ID:id
	    }).done(function (result) {
	    	if(result.status == 0){
		...
		//更新表格
			var columns =[{
	            ...
	            formatter :function(value,row,inde){
	            	...
	            	if(row.ZXJD && row.ZXWD){
	            		className = "successDingwei";
	            	}
	                var s = "<a class='"+className+"'  onclick=dingwei('" + getLonLat(row.ZXJD) +"','"+ getLonLat(row.ZXWD) +"','"+ row.DWMC + "');>定位</a>";
	                return s;
	            }
			...
			$('#hangyeTable').bootstrapTable({
			    method : 'POST',
			    url : "/seimp/shareExchange/gqyjbxx/getGQyjbxxData",
			    columns : columns,
			    //search:true,
	...
	if(value){
		...
		if(arr.length > 0){
			if(arr[0].trim().length>1){
				result = parseFloat(arr[0].trim().substr(0, arr[0].length-1));
			}
		}
		if(arr.length > 1){
			if(arr[1].trim().length>1){
				result = parseFloat(result) + parseFloat((parseFloat(arr[1].trim().substr(0, arr[1].length-1))/60).toFixed(4));
			}
		}
		if(arr.length > 2){
			if(arr[2].trim().length>1){
				result = parseFloat(result) + parseFloat((parseFloat(arr[2].trim().substr(0, arr[2].length-1))/3600).toFixed(4));
			}
		}
...
//全部行业
//省份
function getGPfygkCons(){
	ajaxPost("/seimp/shareExchange/gqyjbxx/getGQyjbxxCons", {}).done(function (result) {
        $("#HYFL").html("");
        $("#QYZCLXMC").html("");
        $("#HYFL").append('<option value="">全部</option>');
        $("#QYZCLXMC").append('<option value="">全部</option>');
        
        for (var i = 0; i < result.data.hyfl.length; i++) {
            var currItem = result.data.hyfl[i];
            $("#HYFL").append('<option value="' + currItem.HYFL + '">' + currItem.HYFL + '</option>');
        }
        
        for (var i = 0; i < result.data.qyzclmc.length; i++) {
            var currItem = result.data.qyzclmc[i];
            $("#QYZCLXMC").append('<option value="' + currItem.QYZCLXMC + '">' + currItem.QYZCLXMC + '</option>');
        }
    })
}
...
function De5(){
	ajaxPost("/seimp/shareExchange/gqyjbxx/getStatisDataByUpdateTime", {
...
function kepGetExcelFile(){
	...
	ajaxPost("/seimp/shareExchange/gqyjbxx/getExcelFile", parameter).done(function (result) {
		//删除等待提示框
