...
$(function(){
    ...
    ajaxPost("/seimp/shareExchange/getDatalistByID", {
		...
		ajaxPost("/seimp/shareExchange/qypfgk/getLastUpdateTime", {
	    	ID:id
	    }).done(function (result) {
	    	if(result.status == 0){
	...
	function De2(){
		//更新表格
			...
			$('#hangyeTable').bootstrapTable({
			    method : 'POST',
			    url : "/seimp/shareExchange/qypfgk/getQypfgkData",
			    columns : columns,
			    //search:true,
...
function getHQyjbxxCons(){
	ajaxPost("/seimp/shareExchange/qypfgk/getQypfgkCons", {}).done(function (result) {
        $("#ZDYLBMC").html("");
        $("#SYND").html("");
        $("#ZDYLBMC").append('<option value="">全部</option>');
        $("#SYND").append('<option value="">全部</option>');
        
        for (var i = 0; i < result.data.zdylbmc.length; i++) {
            var currItem = result.data.zdylbmc[i];
            $("#ZDYLBMC").append('<option value="' + currItem.ZDYLBMC + '">' + currItem.ZDYLBMC + '</option>');
        }
        
        for (var i = 0; i < result.data.synd.length; i++) {
            var currItem = result.data.synd[i];
            $("#SYND").append('<option value="' + currItem.SYND + '">' + currItem.SYND + '</option>');
        }
    })
}
...
function De5(){
	ajaxPost("/seimp/shareExchange/qypfgk/getStatisDataByUpdateTime", {
...
//			XXMC : $("#XXMC").val(),
	...
	ajaxPost("/seimp/shareExchange/qypfgk/getExcelFile", parameter).done(function (result) {
		//删除等待提示框
