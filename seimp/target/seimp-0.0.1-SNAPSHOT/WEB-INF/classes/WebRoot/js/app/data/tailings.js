...
$(function(){
    ...
    ajaxPost("/seimp/shareExchange/getDatalistByID", {
		...
		ajaxPost("/seimp/shareExchange/tailings/getLastUpdateTime", {
	    	ID:id
	    }).done(function (result) {
	    	if(result.status == 0){
	...
	function De2(){
		//更新表格
			var columns =[{
			    ...
			    valign: 'middle' 
		            ...
		            formatter :function(value,row,inde){
		            	...
		            	if(row.COORDINATE){
					    	...
					    	if(arr1.length > 1){
					    		...
					    		if(arr2.length > 1){
					    			result = arr2[1];
					    			if(result != ""){
					    				lat = result.substr(0, result.length-1);
					    			}
					    		}
					    	}
		                ...
		                var s = "<a class='"+className+"'  onclick=dingwei('" + lon +"','"+ lat +"','"+ row.TAILINGSNAME + "');>定位</a>";
		                return s;
		            }
		        }];
			//销毁表格
			...
			$('#hangyeTable').bootstrapTable({
			    method : 'POST',
			    url : "/seimp/shareExchange/tailings/getTailingsData",
			    columns : columns,
			    //search:true,
...
//			    onlyInfoPagination:false,
			    queryParams: function queryParams(params) {   //设置查询参数  
			    	...
			    	var data =  JSON.stringify({
			    		...
			    		TAILINGSNAMEPar : $("#TAILINGSNAMEPar").val(),//尾矿库名称
			            DETAILEDADDRESSPar : $("#DETAILEDADDRESSPar").val(),//详细地址
			            ENTERPRISENAMEPar : $("#ENTERPRISENAMEPar").val(),//所属企业名称
			            MINERALTYPEPar : $("#MINERALTYPEPar").val(),//矿物种类
			            BASINPar : $("#BASINPar").val(),//所属流域
			            LEVELPar : $("#LEVELPar").val()//等别
				    });	    	
					datas.data = data;
				    return datas;  
				    console.log(datas)
...
function De1(){
	...
	ajaxPost("/seimp/shareExchange/tailings/getStatisDataOfHyname", {}).done(function (res) {
...
//    		        boundaryGap: [0, 0.01],
    ...
    //判断用户级别
	var staUrl = "/seimp/shareExchange/tailings/getStatisDataOfProvince";
	var staYAxisName = "省份名称";
	var userLevel = sessionStorage["userLevel"];
	if(userLevel && userLevel=="2"){
		staUrl = "/seimp/shareExchange/tailings/getStatisDataOfCity";
		staYAxisName = "市区名称";
		$("#staDivName").html("<i></i>按市区统计尾矿库");
	}
...
function De5(){
	ajaxPost("/seimp/shareExchange/tailings/getStatisDataByUpdateTime", {
...
function kepGetExcelFile(){
	...
	ajaxPost("/seimp/shareExchange/tailings/getExcelFile", parameter).done(function (result) {
		//删除等待提示框
