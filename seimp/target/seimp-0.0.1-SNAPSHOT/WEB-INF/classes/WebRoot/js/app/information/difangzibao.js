...
//		 featureLayer.setLabelingInfo([labelClass]);
		...
		featureLayer.on("graphic-node-add",function(){
		 ...
		 ajaxPost('/seimp/pic/getKeyIndustry1.do',json).done(function(result){
...
//		 featureLayer.setLabelingInfo([labelClass]);
		...
		featureLayer.on("graphic-node-add",function(){
		 ...
		 ajaxPost('/seimp/pic/getKeyIndustry2.do',json).done(function(result){
			if(result.status==0){//请求成功
				var data = result.data;
				if(result.data.length>0){
...
function initCounty_2(){
	...
	require(["esri/layers/GraphicsLayer","esri/InfoTemplate",
		...
		ajaxPost("/seimp/pic/getKeyIndustry3.do", json).done(function(result){
...
//				}]
				...
				function getContent(graphic){
					...
					'<p><code style="cursor:pointer;color:#038AEC;"><a>环统数据</a></code><code style="cursor:pointer;color:#038AEC;"><a>污染源普查数据</a></code><code style="cursor:pointer;color:#038AEC;"><a>排污许可证数据</a></code></p>';
				}
