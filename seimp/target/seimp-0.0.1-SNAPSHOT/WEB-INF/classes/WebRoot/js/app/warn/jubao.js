...
//		 featureLayer.setLabelingInfo([labelClass]);
		...
		featureLayer.on("graphic-node-add",function(){
		 ...
		 ajaxPost('/seimp/warn/getJbyj1.do',json).done(function(result){
			if(result.status==0){//请求成功
...
//		 featureLayer.setLabelingInfo([labelClass]);
		...
		featureLayer.on("graphic-node-add",function(){
		 ...
		 ajaxPost('/seimp/warn/getJbyj2.do',json).done(function(result){
			if(result.status==0){//请求成功
				if(result.data.length>0){
					var data = result.data;
					//处理数据，获取各市数量的最大值最小值
...
//			}
	...
	var json = {industry:mapIndustry,code:cityCode};
	ajaxPost("/seimp/warn/staYQ123691FO1.do", json).done(function(data){
		if(data.status==0){//请求成功
//			if(data.data.length>0){
	...
	require([
		 		...
		 		graphicsLayer.on("click",function(evt){
					...
					map.centerAt(evt.graphic.geometry);
		 			...
		 			ajaxPost("/seimp/warn/staYQ123691FO2.do", json).done(function(result){
		 				if(result.status==0){
		 					var data = result.data;
		        			if(data.length==1){
		        				var html = 
											'<p>举报时间:<code>' + data[0].REPORT_TIME + '</code></p>'+
											'<p>举报对象:<code>' + data[0].REPORT_DEPT_NAME!=null&&data[0].REPORT_DEPT_NAME!="(null)"?data[0].REPORT_DEPT_NAME:"暂无数据" + '</code></p>'+
		        	        	...
		        	        	$(".close").click(function(){
		        				 ...
		        				 newsArr = data;
			           			 ...
			           			 for (var i = 0; i < count; i++) {
			           				var content = newsArr[i].REPORT_DEPT_NAME!=null&&newsArr[i].REPORT_DEPT_NAME!="(null)"?newsArr[i].REPORT_DEPT_NAME:"暂无数据";
									...
									html += "<tr><td class='tdstyle'>"+(i+1)+"</td><td><a href='/seimp/index.html#yuqingDetails?nids="+newsArr[i].REPORT_ID+"&type=jubao' target='_blank' title='"+newsArr[i].REPORT_DEPT_NAME+"'>"+content+"</a></td><td>"+newsArr[i].REPORT_TIME.substr(0, 10)+"</td></tr>";
			           			 }
			           			 html +="</table>";
...
function jubaoTable(data){
	console.log(data)
	if ( data != 0 ) {
		isSelect =1;
		console.log()
		if (data.length>0){
			var headHtml = "<thead> <tr><td>序号</td><td>地区</td><td>预报预警信息数量(条)</td></tr></thead><tbody>";
			var num = 1;
