...
function allPublic(){
	...
	app.pesticideLayer.on("click", function(evt){
		...
		if(graphic){
			if(graphic.attributes.graType && graphic.attributes.graType=="pesticideGra" && graphic.attributes.ID ){
				//农药厂点击高亮
				selectPesticideOrganId = graphic.attributes.ID;
				changeMarkerPicUrl(app.pesticideLayer, "ID", graphic.attributes.ID, "", "_high");
				//农药厂点击，弹出提示框
				pesticideClick(evt);
			}
		}
	...
	app.pesticideLayer.on("mouse-over", function(evt){
		...
		if(graphic){
			if(graphic.attributes.graType && graphic.attributes.graType=="pesticideGra"){
				...
				var mingcheng = graphic.attributes.XIANQIMINGCHENG == null ? "" : graphic.attributes.XIANQIMINGCHENG;
				addTextGraphicToLayer(mingcheng, graphic.geometry, app.map.graphics, "pesticideNameTextGra");
			}
		}
	...
	//污染地块图层
	app.wrdkLayer = new dong.GraphicsLayer({is:'wrdkLayer'});
	...
	app.map.addLayer(app.wrdkLayer);
	
	pes.searchPar.jgmco = $("#jgmco").val();
	pes.searchPar.status = $("#status").val();
	pes.searchPar.province = $("#province option[value='"+$("#province").val()+"']").attr("selectValue");
	pes.searchPar.city = $("#city option[value='"+$("#city").val()+"']").attr("selectValue");
	pes.searchPar.county = $("#county option[value='"+$("#county").val()+"']").attr("selectValue");
...
function addPesticideGraphic(){
	...
	ajaxPost("/seimp/pesticide2/getAllPesticideData", pes.searchPar).done(function (result) {
        if(result.status == 0){
        	var data = result.data;
        	//需要添加到gra上的属性名称
        	var attrColumnArr = ["XIANQIMINGCHENG", "ID"];
        	//添加多个图标点到地图上
...
//            $("#title").attr("title",data.JGMC0);
            //内容
            var html = '<div class="rows"><div class="rows" style="overflow:auto;height:210px;"><div class="rows"><div class="row"><div class="col-sm-3 text-right" style="width: 30%;padding-left: 0;">企业名称：</div><div class="col-sm-9 text-left" style="width: 70%;padding-left: 0;">'+handleValue(data.XIANQIMINGCHENG)+'</div></div>' +
                '<div class="row"><div class="col-sm-3 text-right" style="width: 30%;padding-left: 0;">省份：</div><div class="col-sm-9 text-left" style="width: 70%;padding-left: 0;">'+handleValue(data.SHENG)+'</div></div>' +
                '<div class="row"><div class="col-sm-3 text-right" style="width: 30%;padding-left: 0;">市区：</div><div class="col-sm-9 text-left" style="width: 70%;padding-left: 0;">'+handleValue(data.SHI)+'</div></div>' +
                '<div class="row"><div class="col-sm-3 text-right" style="width: 30%;padding-left: 0;">县区：</div><div class="col-sm-9 text-left" style="width: 70%;padding-left: 0;">'+handleValue(data.XIAN)+'</div></div>' +
                '<div class="row"><div class="col-sm-3 text-right" style="width: 30%;padding-left: 0;">企业地址：</div><div class="col-sm-9 text-left" style="width: 70%;padding-left: 0;">'+handleValue(data.XIANQIDIZHI)+'</div></div>' +
                '<div class="row"><div class="col-sm-3 text-right" style="width: 30%;padding-left: 0;">原企业名称：</div><div class="col-sm-9 text-left" style="width: 70%;padding-left: 0;">'+handleValue(data.YUANQIYEMINGCHENG)+'</div></div>' +
                '<div class="row"><div class="col-sm-3 text-right" style="width: 30%;padding-left: 0;">原企业地址：</div><div class="col-sm-9 text-left" style="width: 70%;padding-left: 0;">'+handleValue(data.YUANQIDIZHI)+'</div></div>' +
                ...
                '<div class="row"  onclick=showDetailsModal2("' + data.ID +'","'+ data.XIANQIMINGCHENG +'")><div class="col-sm-3 text-right" style="width: 30%;padding-left: 0;"></div><div class="col-sm-9 text-left" style="width: 70%;padding-left: 0;"><span style="color:#00A2DA;cursor:pointer;"><a href="javascript:void(0);">详情信息</a></span></div></div>' +
                ...
                '<button type="button" style="margin-right:2%;" class="btn btn-primary" onclick=ljwrdk('+data.LON+','+data.LAT+',"'+data.XIANQIMINGCHENG+'")>临近污染地块分布</button>' +
                '<button type="button" style="margin-right:2%;" class="btn btn-primary" onclick=jsxmhpfx('+data.LON+','+data.LAT+',"'+data.JGMC0+'")>环评项目分布</button>' +
                '<button type="button" class="btn btn-primary" onclick=tiandituAround('+data.LON+','+data.LAT+',"'+data.JGMC0+'")>敏感点分布</button></div>'+
            	'</div>';
...
//            $(".yj_table").css("minHeight",$(".yj_table .table-body .rows").height()+50);
            //地图弹出框
            mapinfoWindow(data.XIANQIMINGCHENG,html,evt);
		}
	})
...
function changeBackAndMap3(){
	 $('#info_table1 tbody tr').hover(function(evt) {
   	...
   	if(changeMap!=null && changeMap!=""){
   		var changeMapArr = changeMap.split(",");
   		changeMarkerPicUrl(app.wrdkLayer, changeMapArr[0], changeMapArr[1], "", "_high");
   	}
   }, function(evt) {
       $('#info_table1 tbody tr').css("background","none")
       var changeMap = $(this).attr("changeMap");
		...
		$.ajax({
			...
			success:function(result){
				...
				if (result && result!="null" && result.count > 0){
	        		...
	        		if (result && result!="null" && result.keyWord){
	        			toastr.warning(result.keyWord+"未查询到结果")
	        		}else{
	        			toastr.warning("查询结果为空")
	        		}
	        	}
				//加上中心点和名称
				if(index==2){
...
function handleTiandituData(data, circle, dataCount){
	...
	for (var i = 0; i < data.length; i++) {
		...
		if(circle.contains(point) && currItem.name.indexOf('门')==-1){
			result.push(data[i]);
		}
...
function ljwrdk(lon, lat, name){
    ...
    var extent = circle.getExtent();
	//请求后台
	ajaxPost("/seimp/pesticide2/getWrdkAroundPesticide", {
		minLon : extent.xmin,
        maxLon : extent.xmax,
        minLat : extent.ymin,
        maxLat : extent.ymax,
		...
		if(result.status == 0){
			var data = result.data;
			data = removeNotInCircleOfWrdk(data, circle);
			if(data.length > 0){
				//地图图层操作
				app.pesticideLayer.setVisibility(false);
				app.circleLayer.clear();
				...
				app.map.setExtent(extent);
				addOneCircleToMap(circle, dong.circleLength, app.circleLayer);
				var attrColumnArr = ["WRDKZXJD", "WRDKZXWD","WRDKID"];
				addPicPoinsToMap(data, "WRDKZXJD", "WRDKZXWD", "4326", app.wrdkLayer, "img/dian/wurandikuai.png", "32", "32", 0, 16, "wrdkGra", attrColumnArr);
				addWrdkToDiv(data, lon, lat, name);
				appOnePicPointToMap(lon, lat, name, "img/dian/nongyao_high.png", app.yzconsLayer, "wrdkGra")
			}else{
				//未查询到结果
				toastr.warning("临近污染地块分析未查询到结果");
			}
		}
	})
}
 ...
 * @returns {Array}
 */
function removeNotInCircleOfWrdk(data, circle){
	var result = [];
	for (var i = 0; i < data.length; i++) {
		var currItem = data[i];
		var point = new dong.Point(currItem.WRDKZXJD, currItem.WRDKZXWD);
		if(circle.contains(point)){
			result.push(data[i]);
		}
	}
 ...
 * @param data
 */
function addWrdkToDiv(data, lon, lat, jgmco){
	var html = '<table id="info_table1" style="width: 100%;height: auto;" class="table table-bordered wrdk-table"><thead><tr>'
		+'<th  width="10%">序号</th>'
		+'<th width="30%">污染地块名称</th>'
		+'<th width="23%">类型</th>'
		+'<th width="23%">所处阶段</th>'
		+'<th width="30%">操作</th></tr></thead><tbody id="hp_boty">';
	var hengyeArr = [];
	for (var i = 0; i < data.length; i++) {
		var currItem = data[i];
		//名称
		var wrdkmc = handleNullValue(currItem.WRDKMC);
		var wrdkmcLess = wrdkmc;
		if(wrdkmc.length > 8){
			wrdkmcLess = wrdkmc.substr(0, 8) + "...";
		}
		//污染地块污染类型
		var pollueted = "";
		if(currItem.POLLUETED){
			if("0"==currItem.POLLUETED){
				pollueted = "非污染地块";
			}
			if("1"==currItem.POLLUETED){
				pollueted = "污染地块";
			}
		}else{
			pollueted = "疑似污染地块";
		...
		var scjd = "";
		if(currItem.SCJDBM){
			scjd = scjdObj[currItem.SCJDBM];
		}
		
		html += '<tr changeMap="WRDKID,'+currItem.WRDKID+'">'
				+'<th>'+(i+1)+'</th>'
				+'<th title="'+wrdkmc+'">'+wrdkmcLess+'</th>'
				+'<th >'+pollueted+'</th>'
				+'<th >'+scjd+'</th>'
				+'<th><a onclick=showWrdkDetails("' + currItem.WRDKBM +'","'+ currItem.WRDKMC +'")>详情</a></th></tr>';
	}
	html+= '</tbody></table>';
	$("#tongjituDiv #title").html("临近污染地块分布清单");
    $("#tongjituDiv .table-body").html(html);
    //表格与地图联动
    changeBackAndMap3();
...
function getIndexSumValue(){
	ajaxPost('/seimp/pesticide2/getIndexSumValue',{
	}).done(function(result){
        if( result.status == 0 ) {
        	var data = result.data;
...
function getEcharts1(){
	ajaxPost("/seimp/pesticide2/getStatisDataByProduct", {
		isCancle : "1"
    }).done(function (result) {
        if(result.status == 0){
...
function getEcharts2(){
	ajaxPost("/seimp/pesticide2/getStatisDataByProvince", {
    }).done(function (result) {
        if(result.status == 0){
	...
	//更新表格
		var columns =[{
	        ...
	        formatter :function(value,row,inde){
		    	var s = "";
		    	if(row.nameWrdk && row.nameWrdk!="0"){
		    		s = "<a class='successDingwei'  onclick=showNameWrdkTable('" + row.XIANQIMINGCHENG +"');>关联到污染地块</a>";
		    	}else{
		    		s = "<a class='errorDingwei'  );>未关联到污染地块</a>";
		    	}
...
//                var s = "<a class='"+className+"'  onclick=dingwei('" + row.LONGITUDE +"','"+ row.LATITUDE +"','"+ row.PROJECTNAME + "');>定位</a>";
            	var s = "<a class='btn btn-info'    onclick=showDetailsModal2('" + row.ID +"','"+ row.XIANQIMINGCHENG.replace(/^\s+|\s+$/g,"") +"') title='详细信息'><i class='iconfont icon-xxxx'></i></a>";
//                var s = "<a   onclick=showDetailsModal2('" + row.ORGANID +"','"+ row.JGMC0 +"');>详情</a>";
		...
		$('#pesticideTable').bootstrapTable({
		    method : 'POST',
		    url : "/seimp/pesticide2/getPesticidePageData",
		    columns : columns,
		    //search:true,
...
function handlePesticideData(columnMode){
	...
	var organIDs = [];
	ajaxPost("/seimp/pesticide2/getPesticideAndYzcons", {
    }).done(function (result) {
        if(result.status == 0){
        	var data = result.data;
        	...
        	if(organIDs.length > 0){
        		...
        		ajaxPost("/seimp/pesticide2/updatePesticideL", {
...
function showDetailsModal2(ORGANID,JGMC0){
	...
	showDetailsModal3(JSON.stringify(enterPriseInfo), JGMC0, "views/data/detailIframe/pesticideLandDetails2.html")
}

/**
 * 临近污染地块分析结果，污染地块结果
 */
function showWrdkDetails(WRDKBM, WRDKMC){
	var enterPriseInfo = {
			WRDKBM:WRDKBM
		};
	showDetailsModal3(JSON.stringify(enterPriseInfo), WRDKMC, "views/data/detailIframe/dataDeDetails.html")
}
...
//显示污染地块表格
function showNameWrdkTable(qymc){
	$("#myModal5").modal('toggle');
	$("#myModalLabel5").text(qymc);
...
//		class: 'last_th',
        formatter :function(value,row,inde){
            var s = "&nbsp;&nbsp;&nbsp;&nbsp;<a   onclick=showWrdkDetails('" + row.WRDKBM +"','"+ row.WRDKMC +"'); >详情</a>";
            return s;
        }
	}];
	//销毁表格
	$('#nameWrdkTable').bootstrapTable('destroy');
	//生成表格
	$('#nameWrdkTable').bootstrapTable({
	    method : 'POST',
	    url : "/seimp/pesticide2/getWrdkByQymc",
	    columns : columns,
	    //search:true,
	    //classes:'table-no-bordered',	//消除表格边框
	    iconSize : "outline",
...
function getLeibieList(){
	ajaxPost("/seimp/pesticide2/getLeibieList", {
    }).done(function (result) {
        if(result.status == 0){
        	var data = result.data;
        	...
        	for (var i = 0; i < data.length; i++) {
        		var currItem = data[i];
				html += "<option value='"+currItem.CHANPINLEIBIE+"'>"+currItem.CHANPINLEIBIE+"</option>";
			}
        	$("#status").html(html);
        }
    })
...
function update666Table(){
	...
	var columns =[{
        ...
        formatter :function(value,row,inde){
	    	var s = "";
	    	if(row.nameWrdk && row.nameWrdk!="0"){
	    		s = "<a class='successDingwei'  onclick=showNameWrdkTable('" + row.YUANQIYEMINGCHENG +"');>关联到污染地块</a>";
	    	}else{
	    		s = "<a class='errorDingwei'  );>未关联到污染地块</a>";
	    	}
	...
	//销毁表格
	$('#666Table').bootstrapTable('destroy');
	//生成表格
	$('#666Table').bootstrapTable({
	    method : 'POST',
	    url : "/seimp/pesticide2/getPesticidePageData",
	    columns : columns,
	    //search:true,
...
function updateDdtTable(){
	...
	//销毁表格
	$('#ddtTable').bootstrapTable('destroy');
	//生成表格
	$('#ddtTable').bootstrapTable({
	    method : 'POST',
	    url : "/seimp/pesticide2/getPesticidePageData",
	    columns : columns,
	    //search:true,
