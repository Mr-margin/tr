...
var qiYeIndustryArr = [{id: "有色金属", name: "有色金属矿采选"}, {id: "有色金属冶炼", name: "有色金属冶炼"}, {id: "石油开采", name: "石油开采"},
    {id: "石油加工", name: "石油加工"}, {id: "化工", name: "化工"}, {id: "焦化", name: "焦化"}, {id: "电镀", name: "电镀"}, {
        id: "制革",
        name: "制革"
    }];
...
/*改变时间范围重新加载地图*/
function gotoDate() {
    var minT = $("#minTotal").val();
    var maxT = $("#maxTotal").val();
    if(parseInt(minT)>parseInt(maxT)){
        return  toastr.info("投资总额最小值不能大于最大值");
    }
    var minE = $("#minEp").val();
    var maxE = $("#maxEp").val();
    if(parseInt(minE)>parseInt(maxE)){
        return toastr.info("环保投资最小值不能大于最大值");
    }
        initMapOfProvince();
...
function updatePieLayer() {
    //加载分省新增污染企业饼图
    var json = {
        timePar: timePar, industryPar: industryPar, type: xiangMuType,
        startTime: $("#datetimeStart").val(), endTime: $("#datetimeEnd").val(),
        minPROJECTINVEST: $("#minTotal").val(), maxPROJECTINVEST: $("#maxTotal").val(),
...
function addChart(data) {
    ...
    require([
        ...
        "dojo/domReady!"
    ], function (Map, GraphicsLayer, SpatialReference, webMercatorUtils, Point, FeatureLayer, ArcGISDynamicMapServiceLayer,
...
function getMapRightEchartsData() {
    var json = {
        ...
        minPROJECTINVEST: $("#minTotal").val(),
        maxPROJECTINVEST: $("#maxTotal").val(),
        minENVIRONINVEST: $("#minEp").val(),
        maxENVIRONINVEST: $("#maxEp").val()
    };
    ajaxPost('/seimp/warn/newPollutingEnterprise1.do', json).done(function (data) {
        if (data.status == 0) {//请求成功
//			if(data.data.length>0){
...
function updateTableData() {
    ...
    ajaxPost("/seimp/warn/newPollutingEnterpriseDetail1.do", json).done(function (result) {
            ...
            if(result.data.total==0){
                ...
                var options = {
                    ...
                    itemTexts: function (type, page, current) { //修改显示文字
                        ...
                        ajaxPost("/seimp/warn/newPollutingEnterpriseDetail1", {
                            timePar: timePar,
                            industryPar: industryPar,
                            provinceFIDPar: provinceFIDPar,
                            type: xiangMuType,
...
function updateTable(data) {
	if(xiangMuType == 1){//建设项目
		var headHtml = "<td width='7%'>序号</td><td width='23%'>项目名称</td><td width='23%'>行业类别</td><td width='7%'>对接省份</td>" +
				"<td width='7%'>总投资</td><td width='7%'>环保投资</td><td width='23%'>批复时间</td>";
	    var bodyHtml = "";
	    if (data.length > 0) {
	        for (var i = 0; i < data.length; i++) {
	            var currItem = data[i];
	            bodyHtml += "<tr><td width='7%'>" + (i + 1) + "</td>" +
	            		"<td width='23%' style='white-space: nowrap;text-overflow: ellipsis;overflow: hidden; width:200px;'><a style='color:black;' title="+ currItem.PROJECTNAME +">" + currItem.PROJECTNAME + "</a></td>" +
	            		"<td width='23%'>" + currItem.EIAMANAGENAME + "</td>" +
	            		"<td width='7%'>" + currItem.PROVINCENAME + "</td>" +
	            		"<td width='7%'>" + currItem.ENVIRONINVEST + "</td>" +
	            		"<td width='7%'>" + currItem.PROJECTINVEST + "</td>" +
	            		"<td width='23%'>" + currItem.APPROVALDATE + "</td>" +
	            		"</tr>"
	        }
	    }
	    ...
	    var headHtml = "<td width='10%'>序号</td><td width='40%'>项目名称</td><td width='40%'>行业类别</td><td width='10%'>对接省份</td>";
	    var bodyHtml = "";
	    if (data.length > 0) {
	        for (var i = 0; i < data.length; i++) {
	            var currItem = data[i];
	            bodyHtml += "<tr><td width='10%'>" + (i + 1) + "</td><td width='40%' style='white-space: nowrap;text-overflow: ellipsis;overflow: hidden; width:200px;'><a style='color:black;' title="+ currItem.PROJECTNAME +">" + currItem.PROJECTNAME + "</a></td><td width='40%'>" + currItem.EIAMANAGENAME + "</td><td width='10%'>" + currItem.PROVINCENAME + "</td></tr>"
	        }
	    }
	    $("#tongjibiao thead tr").html(headHtml);
	    $("#tongjibiao tbody").html(bodyHtml);
...
//						addChart(data.data);
                    ...
                    for (var i = 0; i < data.length; i++) {
                        var currItem = data[i];
                        if (minCount <= currItem.NUM && currItem.NUM < minCount + count * 1) {
                            renderer.addValue(currItem.PROVINCENAME, symbol1);
                        } else if (minCount + count * 1 <= currItem.NUM && currItem.NUM < minCount + count * 2) {
                            renderer.addValue(currItem.PROVINCENAME, symbol2);
                        } else if (minCount + count * 2 <= currItem.NUM && currItem.NUM < minCount + count * 3) {
                            renderer.addValue(currItem.PROVINCENAME, symbol3);
                        } else if (minCount + count * 3 <= currItem.NUM && currItem.NUM < minCount + count * 4) {
                            renderer.addValue(currItem.PROVINCENAME, symbol4);
                        } else if (minCount + count * 4 <= currItem.NUM && currItem.NUM <= minCount + count * 5) {
                            renderer.addValue(currItem.PROVINCENAME, symbol5);
                        } else if (minCount + count * 5 <= currItem.NUM && currItem.NUM <= minCount + count * 6) {
            ...
            for (var j = 0; j < data.length; j++) {
                ...
                if (currFeaGraphic.attributes.PROV_CODE == currItem.code) {
                    ...
                    var attributes = {
                        ...
                        provinceName: currItem.PROVINCENAME
                    }
                    var point = currFeaGraphic.geometry.getCentroid();
                    //处理河北省
                    if (currFeaGraphic.attributes.PROV_CODE == "130000") {
                    ...
                    var symbolSize = 24;
                    if (currItem.NUM.toString().length == 1) {
                        symbolSize = 24;
                    } else if (currItem.NUM.toString().length == 2) {
                        symbolSize = 30;
                    } else if (currItem.NUM.toString().length == 3) {
                        symbolSize = 40;
                    } else if (currItem.NUM.toString().length == 4) {
                        symbolSize = 50;
                    } else if (currItem.NUM.toString().length == 5) {
                        symbolSize = 60;
                    } else {
                        symbolSize = currItem.NUM.toString().length * 12;
                    }
                    var symbol1 = new PictureMarkerSymbol("../../img/information/number.png", symbolSize, symbolSize).setOffset(0, symbolSize / 2);
                    var symbol2 = new TextSymbol(currItem.NUM).setOffset(0, symbolSize / 2 - 5).setColor(new Color([255, 255, 255, 1]));//.setHaloSize(3).setHaloColor(new Color([255,255,255]));
...
$('.large').click(function(){
	if($(this).attr('data') == 1){
		...
		$('#main_tabletitle').css('width','100%')
		$('#main_tabletitle').removeClass('caonidaye')
		$(this).attr('data','2')
	}else{
		$(this).attr('data','1')
		$('#tongjituDiv').css({
