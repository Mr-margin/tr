 ...
 */
(function () {
    var M, K, L = Math.abs, ah = Math.sin, w = Math.cos, s = Math.max, aD = Math.min, ap = Math.ceil, F = Math.sqrt,
        at = Math.pow, h = {}, l = {}, m = {
            0: "0,",
            1: "17,",
            2: "34,",
    ...
    function X(aQ, aJ, j, aU, aT, aR, aP, aL, aI, aS, aK, aO) {
        var aN = aT + (aL || 0) + (aI.length && aI[0] < 0 ? L(aI[0]) : 0),
            aH = aR + (aL || 0) + (aI.length && aI[1] < 0 ? L(aI[1]) : 0), aM, aV;
        aQ.font = aJ;
        aQ.textBaseline = "top";
        aQ.fillStyle = j;
    ...
    function aC(aU, aP, aV, aZ, aQ, aO, aM, aS, aK, aL) {
        var aI = aP + ((2 * aS) + aO) * aZ, aR = aV + ((2 * aS) + aO) * aZ, aJ = P(aI, aR), aY, aX, aH, aW, j, a0, aT,
            aN;
        if (!aJ) {
            return null
        }
    ...
    function Z(aN, aT, aP, aJ, aR, aS, aI) {
        var aU = L(aI[0]), aO = L(aI[1]), aK = aT + (aU > aS ? aU + aS : aS * 2) * aJ,
            j = aP + (aO > aS ? aO + aS : aS * 2) * aJ, aM = aJ * ((aS || 0) + (aI[0] < 0 ? aU : 0)),
            aH = aJ * ((aS || 0) + (aI[1] < 0 ? aO : 0)), aL, aQ;
        aL = P(aK, j);
        if (!aL) {
            return null
        }
    ...
    function t(aT, aL, aR) {
        var aS = parseInt(aT.toString().length * aR), aK = parseInt(aR * 2 * aT.length), aI = P(aS, aK), aO, j, aJ, aN,
            aQ, aP, aH, aM;
        if (!aI) {
            return null
        }
    ...
    function ab(aH) {
        var aK = C.getElementById(aH), i = aK.getBoundingClientRect(), aN = C.documentElement, aL = C.body, aM = window,
            aI = aM.pageXOffset || aN.scrollLeft, aO = aM.pageYOffset || aN.scrollTop,
            aJ = aN.clientLeft || aL.clientLeft, j = aN.clientTop || aL.clientTop;
    ...
    x.DrawColourImage = function (aM, aP, aN, aQ, aL, i, aT, j, aO) {
        var aR = aM.canvas, aJ = ~~s(aP, 0), aI = ~~s(aN, 0), aK = aD(aR.width - aJ, aQ) + 0.5 | 0,
            aS = aD(aR.height - aI, aL) + 0.5 | 0, aH;
        if (p) {
    ...
    c.DrawImage = function (aJ, aQ, aI, aL) {
        var aN = this.x, aK = this.y, aR = this.sc, j = aL || this.fimage, aO = this.w, aH = this.h, aM = this.alpha,
            aP = this.shadow;
        aJ.globalAlpha = aM;
    ...
    c.DrawImageIE = function (aJ, aN, aI) {
        var j = this.fimage, aO = this.sc, aM = j.width = this.w * aO, aH = j.height = this.h * aO,
            aL = (this.x * aO) + aN - (aM / 2), aK = (this.y * aO) + aI - (aH / 2);
        aJ.setTransform(1, 0, 0, 1, 0, 0);
        aJ.globalAlpha = this.alpha;
    ...
    Q.UpdateTag = function (aH, i) {
        var aK = this.textColour || Y(i, "color"), j = this.textFont || o(Y(i, "font-family")),
    ...
    Q.Weight = function (aN) {
        var aJ = aN.length, aL, aH, aO, aK = [], j, aI = this.weightFrom ? this.weightFrom.split(/[, ]/) : [null],
            aM = aI.length;
        for (aH = 0; aH < aJ; ++aH) {
            aK[aH] = [];
            for (aO = 0; aO < aM; ++aO) {
    ...
    Q.Load = function () {
        var aR = this.GetTags(), aM = [], aP, aQ, aL, aI, aH, j, aJ, aO, aK = [],
    ...
    Q.Draw = function (aR) {
        ...
        var aL = this.canvas, aJ = aL.width, aQ = aL.height, aT = 0, aI = (aR - this.time) * y.interval / 1000,
            aP = aJ / 2 + this.offsetX, aO = aQ / 2 + this.offsetY, aX = this.ctxt, aN, aY, aV, aH = -1,
            aK = this.taglist, aU = aK.length, j = this.frontSelect, aS = (this.centreFunc == aA), aM;
        this.time = aR;
    ...
    Q.Transform = function (aK, i, aM) {
        if (i || aM) {
            var j = ah(i), aL = w(i), aN = ah(aM), aJ = w(aM), aH = new R([aJ, 0, aN, 0, 1, 0, -aN, 0, aJ]),
