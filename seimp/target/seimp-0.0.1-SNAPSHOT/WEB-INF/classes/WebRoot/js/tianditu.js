...
function  liangmian(){
	require(["esri/map", 
        ...
        "dijit/TitlePane",
        "dojo/domReady!"],
function(Map, Scalebar, OverviewMap, dom, 
		Color, parser, Graphic,GraphicsLayer,
		  Point, 
		  Measurement, Legend, 
		 SimpleLineSymbol, on,SimpleFillSymbol
		,Polygon,Query,SimpleMarkerSymbol,SimpleRenderer,ArcGISTiledMapServiceLayer,Print,
		PrintTemplate,InfoTemplate,PrintTask,PrintParameters,Units,DEBubblePopup,ArcGISDynamicMapServiceLayer,IdentifyTask,IdentifyParameters,
		BasemapToggle,webMercatorUtils
) {
        var mdiv = dijit.byId("measurementDiv");
        if (mdiv) {
        	mdiv.destroyRecursive(true);
        }
        ...
        measurement.startup();
        $(".esriMeasurement .dijitContentPane").css("overflow-y","hidden")
...
$("#celiang").click(function(){
	if(map.getLayer("gaosuLayer")){
	 ...
	 var  aaa =document.getElementById("meaDiv").style.display;
     if (aaa =="none"){
    	 $("#meaDiv").show();
    	 $("#tucengDiv").hide();
    	 $("#toumingduDiv").hide();
